...
<?php
	foreach ($payments as $payment){ 
		...
		/* --------------------------------------------------------------------------------------------- */ 
			...
			if(isset($paid_trans[$payment->payment_id]) && sizeof($paid_trans[$payment->payment_id]) > 0){ $i=0;
				
				foreach($paid_trans[$payment->payment_id] as $values){ $i++;
...
<div class="col-md-12">
	<div style="width:820px">
		...
		<?php } else { ?>
			<div style="height: 400px; margin-top: 60px;" >
		<?php } ?>
				<table class="table table-condensed" border="0">
					<tbody>
				       ...
				       <tr style="height: 76px;"> 
				       		<td colspan="3"><?php echo Base_Controller::ToggleLang('Amount (in words)').": ";?> <span  id="td_words_val<?php echo $payment->payment_id;?>" style="text-decoration: underline;"></span>
			...
			<?php }?>
				<table class="table table-condensed">
					<tbody>
				       ...
				       <?php if($term_paid > 0 && $pending_due > 0) { ?>
				       <tr style="height: 76px;"> 
				       		<td colspan="3"><?php echo Base_Controller::ToggleLang('Amount (in words)').": ";?> <span  id="td_words_val<?php echo $payment->payment_id;?>2" style="text-decoration: underline;"></span> 
				       		<br/><br/><span style="font-style: oblique; font:xx-small; color: gray;"> <?php if($payment_detail != "") echo $payment_detail;?></span>
				       		</td>
				       </tr>
				       <tr>
	...
	document.getElementById('td_words_val'+'<?php echo $payment->payment_id;?>'). innerHTML = document.getElementById('td_words_val'+'<?php echo $payment->payment_id.'2';?>'). innerHTML = "Saudi Riyals " + toWords('<?php echo number_format($form2->payment_amount)?>') + " only";
	</script>
