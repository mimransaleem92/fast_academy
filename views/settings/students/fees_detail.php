	...
	<div class="portlet box blue">
		<div class="portlet-title">
			...
			<div class="actions">
				...
				<?php } 
				$payment_url   = base_url().$model."/add_payment";
				$param_payment = 'student_id='.$form->student_id;
				
				?>
				<a href="javascript:;" class="btn default btn-xs red" <?php echo 'onclick="showInnerBox( [\''. $payment_url .'\', \'_update\', \'addPaymentForm\', \'pending_fee_div\'], \''.$param_payment.'\', {error: function() { alert(\'Could not load form\')}, title: \'Add Payment\'})"'; ?>><i class="fa fa-plus"></i> Payment </a>
				<a class="btn green hide" href="<?php echo base_url().$model.'/payment/?student_id='.$form->student_id;?>"><i class="fa fa-cash"></i> Receive Payment </a>
				<?php } ?>
			</div>
		</div>
				...
				<table class="table table-condensed table-hover">
					<thead>
						<tr>
							...
							<th><?php echo Base_Controller::ToggleLang('Fee Slip #');?></th>
						...
						<?php 
                          ...
                          ?>
	                          <tr>
	                            ...
	                            <td <?php if($payment_data_content != "") echo 'class="popovers" data-trigger="hover" data-container="body" data-placement="top" data-content="'.$payment_data_content.'" data-original-title="Payment Detail"';?> ><?php echo $values->card_number; ?></td>
								...
								<?php } else {
									if($admin_role == 3){
										if($values->mark_delete == 3){
											...
											echo '<a href="javascript:;" class="btn default btn-xs blue-stripe " onclick="window.open(\''.base_url().$model.'/cheque_receipt/'. $values->payment_id.'/?student_id='.$values->student_id.'\', \'_blank\')" href="#">Print</a>';
										}
