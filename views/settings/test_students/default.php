			...
			<table class="table table-condensed table-hover" >
				<thead>
					<tr>
						...
						<th class="hidden-xs hidden-sm"><?php echo Base_Controller::ToggleLang('SR #');?></th>
						<th><?php echo Base_Controller::ToggleLang('Admintion #');?></th>
						<th><?php echo Base_Controller::ToggleLang('Name');?></th>
						<th class="hidden-xs hidden-sm"><?php echo Base_Controller::ToggleLang('Father Name');?></th>
						<th><?php echo Base_Controller::ToggleLang('Course');?></th>
						<th class="hidden-xs hidden-sm"><?php echo Base_Controller::ToggleLang('Mobile #');?></th>
						<th class="hidden-xs hidden-sm"><?php echo Base_Controller::ToggleLang('Father Contact #');?></th>
					</tr>
				</thead>
				<tbody>
				...
				<tr>
					<th class="hidden-xs hidden-sm"></th>
					<th class="hidden-xs hidden-sm"></th>
					<th><input type="text" autocomplete="off" value="<?php echo (isset($_POST['admission_number'])) ? $_POST['admission_number'] : '';?>" onkeyup="onkeyup_search(this, event);" placeholder="<?php if ($div_id == 1) echo Base_Controller::ToggleLang('FB-16-0000'); else echo Base_Controller::ToggleLang('FG-15-0000');?>" name="admission_number" style="width: 120px" class="form-control input-sm"/></th>
					...
					<th class="hidden-xs hidden-sm"><input type="text" autocomplete="off" value="<?php echo (isset($_POST['father_name'])) ? $_POST['father_name'] : '';?>" onkeyup="onkeyup_search(this, event);" placeholder="<?php echo Base_Controller::ToggleLang('Father Name');?>" id="father_name" name="father_name"  style="width: 180px; text-transform: uppercase;" class="form-control input-sm"/></th>
					<th colspan="2">
		        ...
		        <?php 
		            ...
		            ?>
		            <tr>
		            	<td class="hidden-xs hidden-sm" ><input type="checkbox" name="selected_id_<?php echo $i;?>" id="selected_id_<?php echo $i;?>" <?php //if($values->status == 'Issued') echo 'disabled="disabled"';?> value="<?php echo $values->student_id;?>" /></td>
		             	<td class="hidden-xs hidden-sm" onclick="onclick_row('<?php echo $i;?>');" style="color:#888888;text-align:center"><strong><?php echo $i; ?></strong></td>
		              	<td onclick="" nowrap="nowrap"><a href="<?php echo base_url().'test_students/student_profile/'.$values->student_id?>"><?php echo $values->admission_number?></a></td>
		             	<td onclick="onclick_row('<?php echo $i;?>');"><?php echo $values->student_name?></td>
		             	<td class="hidden-xs hidden-sm" onclick="onclick_row('<?php echo $i;?>');"><?php echo $values->father_name; ?></td>	                            
		             	<td onclick="onclick_row('<?php echo $i;?>');"><?php echo $values->course_name." ".$values->section;?></td>
		             	<td class="hidden-xs hidden-sm" onclick="onclick_row('<?php echo $i;?>');"><?php echo $values->cell_phone_father;?></td>
		              	<td class="hidden-xs hidden-sm" onclick="onclick_row('<?php echo $i;?>');"><?php echo $values->cell_phone_mother;?></td>	                          
		        	</tr>
		            <?php 	}
		            }
		    	?></tbody>
