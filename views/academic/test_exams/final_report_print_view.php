...
<div class="col-md-12">
	...
	<?php 
		/* // chess game invention rice game
		$r = 1;
		for($i = 1; $i < 128; $i++){
			$r = 2*$r ;
			echo $r . ', ';
		}*/
		if (!is_null($student_list) && isset($student_list[0])){
			$form = $student_list[0];
		}
		...
		$address_line2 = ($form->gender == 'M' && $form->course_id > 6) ? "license_no_117s" : "license_no_4350140114";
							...
							<table border="0" style="width: 100%; background-color: #eaffea; color: #000;">
								<?php foreach ($header_fields as $field=>$caption){ ?>
										<tr>
											...
											<?php if($field == 'nationality_en') echo ''.$student->nationality_en; 
												  else{ echo $form->$field; }?>
											</td>
										</tr>
									<?php } ?>
								</table>
		 		...
		 		<div class="row" style="padding-top: 21px">
		 			<div class="col-lg-12 col-md-12 col-sm-12" style="">
			 			<table border="1" style="border: 2px solid #000; width: 100%; background-color: #FFF; color: #000;">
			 				<tr style="background-color: #eaffea !important;">
			                ...
			                <?php $total1 = $total2 = $total_obtained = $credit_hours = 0;
			                	  $gpa_subject_total = $gpa_subject_count = $total_gpa_average = 0;
			                	...
			                	if(isset($subject_list) && sizeof($subject_list)>0){
			                		foreach ($subject_list as $row){
			                			...
			                			if(isset($marks[$student->student_id][$row->subject_id][1])){
			                				...
			                				$first_term_total += round($marks[$student->student_id][$row->subject_id][1]);
			                				$row_total += round($marks[$student->student_id][$row->subject_id][1]);
			                				
			                				$total_first += $first_term_total;
		                				}
		                				if(isset($marks[$student->student_id][$row->subject_id][2])){
		                				
		                					$second_term_total += round($marks[$student->student_id][$row->subject_id][2]);
		                					$row_total += round($marks[$student->student_id][$row->subject_id][2]);
			                			...
			                			$weighted_average = ($row_total/($row->total_score))*100;
										$row_gpa = Util::gpa($weighted_average);
										$row_gpa = $row_gpa * $row->credit_hours;
										if($row->subject_id < 24){ $total_gpa_average += $row_gpa; }
			                ...
			                <tr>
			                	...
			                	<td align="center"><?php echo number_format($first_term_total, 2);?></td>
			                	<td align="center"><?php echo number_format($second_term_total, 2);?></td>
			                	<td align="center"><?php echo number_format($row_total, 2);//.'/'.$row->total_score;?></td>
								<td align="center"><?php echo ($row->subject_id > 25) ? '-' : number_format($row_gpa, 2); //echo '--'.$weighted_average;?></td>
			                ...
			                ?>
			                <tr style="background-color: #DCE897 !important">
			                	<td align="center"><b><?php echo Base_Controller::ToggleLang('total');?></b></td>
			                	<td></td>
								...
								<td align="center"><?php echo number_format($total_gpa_average, 2);?></td>
							...
							<?php $final_gpa = $total_gpa_average/$credit_hours;?>
							<tr style="font-weight: bold;background-color: #FFF">
								...
								<td align="center"><?php echo 'GPA '.number_format($final_gpa, 2);?></td>
								<td align="center"></td>
								<td align="center"></td>
							</tr>
		            	</table>
	...
	<div style="width:820px; padding-top: 10px; font-size: 13px; font-family: 'Bodoni MT', Didot, 'Didot LT STD', 'Hoefler Text', Garamond, 'Times New Roman', serif;" >
		<div style="text-align: left; float: left; width:160px;height:30px; margin-top: -10px; margin-left: -3px;">
			<?php echo Base_Controller::ToggleLang('promoted to').': '; echo ($final_gpa >= 1) ? $course[$form->course_id+1]['course_name'] : '--';?>
		</div>
		<div style="text-align: center; float: right; width:160px;height:30px; border:1px solid #040029; margin-top: -15px; margin-right: -7px; padding-top: 5px">
			<?php echo ($final_gpa >= 1) ? Base_Controller::ToggleLang('result_right', 'ar') : '--';?>
		</div>
		<table border="0" width="100%">
			...
			<tr>
				...
				<?php 
				$principal_name = ($form->gender == 'M' && $form->course_id > 6) ? 'Principal Name Boys' : 'Principal Name';
				echo Base_Controller::ToggleLang('name', 'ar').': <span style="width: 100px">' . Base_Controller::ToggleLang($principal_name, 'ar').'</span><br>';?>
				_______________: <?php echo Base_Controller::ToggleLang('stamp_date', 'ar').'<br>&nbsp;<br>&nbsp;';?>
				_______________: <?php echo Base_Controller::ToggleLang('exam_date', 'ar').'<br>&nbsp;<br>&nbsp;';?></td>
				
				<td style="text-align: right;padding-right: 100px" >
				<?php $controller_name = ($form->gender == 'M' && $form->course_id > 6) ? 'controller name Boys' : 'controller name';
				echo Base_Controller::ToggleLang('name', 'ar').': <span style="width: 100px">' . Base_Controller::ToggleLang($controller_name, 'ar').'</span><br>';?>
				________________: <?php echo Base_Controller::ToggleLang('stamp_date', 'ar');?></td>
				<td style="border: 1px solid; height: 60px"></td>
			</tr>
			
			<tr>
				<td style="text-align: center;" ></td>
				<td style="text-align: center; height: 60px"><?php echo Base_Controller::ToggleLang('School Stamp')
											.'<br>'.Base_Controller::ToggleLang('School Stamp', 'ar');?></td>
				<td style="text-align: center;" ></td>
			</tr>
			
		</table>
		<div style="text-align: left; float: left; width:450px;height:30px; margin-top: 100px; margin-left: -3px;">
