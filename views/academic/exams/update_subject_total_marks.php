		<?php echo form_open('students/add_attendance', array('id'=>'marksForm')); ?>
			<table width="100%" cellspacing="0" cellpadding="4" border="0" class="whitebgBorder"> 
				...
				<tr >
					...
					<td width='8%' align="<?php echo $class_left;?>"><?php echo Base_Controller::ToggleLang('Month');?>:</td>
				...
				<?php $total_marks =50; foreach($marks as $m){?>
				      <tr>	
				      	<td><?php echo $m->subject_name?>:</td>
				      	<td><input type="text" class="form-control input-sm" onkeypress="keyPressAllow(event,this, '<?php echo $total_marks?>');" style="text-align: right" id="total_marks<?php echo $m->subject_id?>" name="total_marks<?php echo $m->subject_id?>" value='<?php if(isset($m->total_marks)) echo $m->total_marks;?>' /></td>
				      	<td>&nbsp;</td>
				      </tr>
				<?php }?>      
				<tr class="hide">
