...
class fee_history extends Base_Controller{
	...
	function index(){
		...
		$this->data['form_detail']=$this->Reports_model->get_fee_history($from, $to);
		$this->data['model'] = 'fee_history';
	...
	function search(){		
		...
		$this->data['form_detail']=$this->Reports_model->get_fee_history($from, $to, $_POST);
		$this->load->view('reports/fee_history',$this->data);
	}
	...
	function prints(){
		...
		$this->data['batch_name'] = Base_model::get_batch_name($b);
		$this->data['form_detail']=$this->Reports_model->get_fee_history($from, $to, $_GET);
		$this->data['model'] = 'fee_history';
		$this->load_printhtml_tmpl('reports/fee_history');
	}
	...
	function excel(){
		...
		$form_detail = $this->Reports_model->get_fee_history($from, $to, $_GET);
		$this->data['model'] = 'fee_history';
		...
		//name the worksheet
		$this->excel->getActiveSheet()->setTitle('student history report');
		//set cell A1 content with some text
		$this->excel->getActiveSheet()->setCellValue('A1', 'Student History Report');
		...
		/*---------------------------------------------------------------------------------*/
		$filename='student_history_'.date('Ymd').'.xls'; //save our workbook as this file name
