...
class Test_students extends Base_Controller{
	...
	function __construct()
	{
		...
		$this->load->model('Teststudents_model','',TRUE);
		
	}
	
	function accounts_posting(){
		ini_set('max_execution_time', 600);
		$this->Teststudents_model->accounts_updating();
		echo 'Done';
	}
	
	function add_student_tests(){
	    echo 'start';
	    //$this->Teststudents_model->add_student_tests();
	    echo 'Done';
	}
	
	function index(){
		$this->Teststudents_model->get_due_payments(466);
	...
	function display($row = 0)// used for pagination 
	{
		...
		$config['base_url'] = base_url().'test_students/display/';
		$config['total_rows']=$this->Teststudents_model->get_students_count();
		$this->pagination->initialize($config);
		$this->data['student_list'] = $this->Teststudents_model->get_all_students($row,TRUE);	
		$this->data['page_count'] = $row;
		$this->data['links']=$this->pagination->create_links();
		$this->load_template('settings/test_students/default');
	}
	
	function student_list(){
		
		$this->data['student_list'] = $this->Teststudents_model->get_all_students();
		$this->load_template('settings/test_students/default');
	}
	
	function process_fee(){
		$enrollment_id=$this->uri->segment(3);
		$student = $this->Teststudents_model->transfer_to_students($enrollment_id);
		
		/*$files = $this->Teststudents_model->get_student_files($enrollment_id);
	 	$dir = explode('/', $files[0]->file_path);
	 	$i = sizeof($dir);
	 	$enrollment_admission_number = $dir[$i-2];
	 	$admission_number = $student[1];*/
	 	
		redirect('test_students/student_profile/'.$student[0]."#tabs_3",'location');
	}
	
	function valid_sibling(){
		$refer = $this->uri->segment(3);
		$flag = $this->Teststudents_model->valid_sibling($refer);
		if($flag){
	...
	function load_image(){
		...
		$this->data['form']  =  $this->Teststudents_model->get_students($id);
		
		$this->load_template('settings/test_students/student_image');
	}
	...
	function profilephoto(){
		...
		if(isset($_GET['v'])){
			...
			if(isset($_POST['w'])){
				...
				{
					$this->imgupload($id);
					redirect('test_students/student_profile/'.$id,'location');
				}
				else if($_POST['w'] > 0)
				{
					$this->imgcrop();
		...
		$this->load_template('settings/test_students/profilephoto');
	}
	...
	function save_crop(){
		...
		$this->Teststudents_model->update_image_path($file_name);
        ...
        if(!$result) {
            ...
            $this->data['action_bar'] = array('add'=>'0','update'=>'0','delete'=>'0','edit'=>'0','save'=>'1','view'=>'0','confirm'=>'0','print'=>'0');
			$this->load->helper('form');
	        $this->data['form']  =  $this->Teststudents_model->get_students($student_id);
			$this->load_template('settings/test_students/student_image');
        }
	...
	function add(){		
		...
		if($this->form_validation->run() == TRUE){
			$id = $this->Teststudents_model->insert();
			$d = $this->session->userdata(SESSION_CONST_PRE.'division_id');
			$division_name = ($d == 1) ? 'FB' : 'FG';
			$admission_number = $this->Teststudents_model->get_admission_number($id);
			...
			foreach($_FILES as $key=>$value)
			{
				for($s=0; $s<=$count-1; $s++)
				{
					...
					if ($this->upload->do_upload('files'))
					{
						...
						$data = array('upload_data' => $this->upload->data());
						$this->Teststudents_model->upload_file_log($data['upload_data'], $id, 'Student');
	    			}
					else
					{
			...
			redirect('test_students','location');
		}
		else
		{
			...
			$this->load_template('settings/test_students/add');
		}
	}
	...
	function edit(){
		...
		$id = $this->uri->segment(3);
		$record = $this->data['form']  =  $this->Teststudents_model->get_students($id);
		$params = $record[0]->course_id;
		$this->data['batch_list'] = Base_model::get_all_batches(1);
		$month = date('m');
		$this->data['subject_list'] = $this->Teststudents_model->get_available_subjects($id, $record[0]->batch_id, $month);
		$this->data['subject_selected'] = $this->Teststudents_model->get_monthly_subjects_detail($id, $record[0]->batch_id, $month);
		$this->data['files'] = $this->Teststudents_model->get_student_files($id);
		...
		$this->data['courses_list'] = Base_model::get_all_courses();
		$this->data['country_list'] = Base_model::get_country_list();
		$this->load_template('settings/test_students/edit');
	}
	...
	function school_leaving(){
		...
		$this->load_template('settings/test_students/school_leaving_form');
	}
	...
	function student_profile(){
		...
		$this->data['pre_date']  = $this->Teststudents_model->get_date_pre($curr_date);
		$this->data['next_date'] = $this->Teststudents_model->get_date_next($curr_date);
		
		$id = $this->uri->segment(3);
		if(is_null($id) || $id == 0) $id = 1;
		$this->data['student_id'] = $id;
		$this->data['form'] = $student  =  $this->Teststudents_model->get_students($id);
		$this->data['guardian']  =  null;//Base_model::get_guardians($id);
		$this->data['previous_data']  =  Base_model::get_previous_data($id); 
		$this->session->set_userdata('student_id', $id);
		
		$detail_list = $this->Teststudents_model->get_attendance($id, $curr_year_month);
		$app_arr = array();
		...
		$this->data['outstanding']  =  null;//$this->Teststudents_model->get_students_outstandings($id, $batch_id, $active_flag);
		$this->data['pending_fee_list']  =  null;//$this->Teststudents_model->get_students_fee_pending_detail($id, $batch_id, $active_flag);
		$this->data['fee_paid_list']  =  null;//$this->Teststudents_model->get_students_fee_paid_detail($id, $batch_id);
		//$this->data['assesment_list'] = Base_model::get_student_assesments($id);
		$this->data['term_list'] = $this->Teststudents_model->get_term_list();
		$this->data['subject_list'] = $row = $this->Teststudents_model->get_subjects_info($student[0]->course_id);
		...
		$mark_detail = $this->Teststudents_model->get_student_marks($student[0]->student_id, $student[0]->course_id, $student[0]->section, $batch_id, '1');
		$marks = array();
		...
		$this->data['voucher_list']  =  null;//$this->Teststudents_model->get_students_vouchers($id, $batch_id); // For refund details
		$this->data['country_list'] = Base_model::get_country_list();
		$this->data['subjects'] = $row = $this->Teststudents_model->get_monthly_subjects_detail($student[0]->student_id, $batch_id, $dt[4]);
		
		$this->data['subject_list'] = $this->Teststudents_model->get_available_subjects($student[0]->student_id, $batch_id, $dt[4]);
		$this->data['subject_selected'] = $this->Teststudents_model->get_monthly_subjects_detail($student[0]->student_id, $batch_id, $dt[4]);
		
		$time_elapsed_secs = microtime(true) - $start;
		$this->data['time_elapsed'] = $time_elapsed_secs;
		$this->load_template('settings/test_students/profile');
	}
	...
	function update_monthly_subjects(){
		...
		$month = $_POST['month'];
		$this->Teststudents_model->update_monthly_subjects($id, $batch_id, $course_id, $month);
	...
	function get_student_term_marks(){
		$id = $_POST['student_id'];
		$term = $_POST['term_id'];
		$student  =  $this->Teststudents_model->get_students($id);
		$term_list = $this->Teststudents_model->get_term_list();
		...
		$this->data['col_total'] = $row[0]->total;
		$batch_id = $student[0]->batch_id;
		$mark_detail = $this->Teststudents_model->get_student_marks($id, $student[0]->course_id, $student[0]->section, $batch_id, $term);
		$marks = array();
		foreach ($mark_detail as $m){
			$marks[$m->student_id][$m->subject_id][0] = $m->field1;
			$marks[$m->student_id][$m->subject_id][1] = $m->field2;
		...
		$this->load->view('settings/test_students/assesment_sheet',$this->data);
	}
	...
	function get_fee_paid(){
		...
		$id = $_POST['student_id'];
		$batch_id = $_POST['batch_id'];
		$this->data['fee_paid_list']  =  $this->Teststudents_model->get_students_fee_paid_detail($id, $batch_id);
		$this->data['voucher_list']  =  $this->Teststudents_model->get_students_vouchers($id, $batch_id); // For refund details echo "Hi...";
	
		$this->load->view('settings/test_students/fee_paid_list',$this->data);
	}
	...
	function get_fee_pending(){
		...
		$id = $_POST['student_id'];
		$batch_id = $_POST['batch_id'];
		$this->data['outstanding']  =  $this->Teststudents_model->get_students_outstandings($id, $batch_id);
		$this->data['pending_fee_list']  =  $this->Teststudents_model->get_students_fee_pending_detail($id, $batch_id);
	
		$this->load->view('settings/test_students/fee_pending_list',$this->data);
	}
	...
	function payment(){
		...
		if($fee_payment_allowed == 'N' || $verified ){
			...
			if(isset($_GET['student_id']) && !empty($_GET['student_id'])){
				redirect('test_students/student_profile/'.$_GET['student_id']."?e=".$err,'location');
			}else{
		...
		$batch_id = $student[0]->batch_id;
		$this->data['outstanding']  =  $this->Teststudents_model->get_students_outstanding_details($id, $batch_id);
		$this->data['pending_fee_list']  =  $this->Teststudents_model->get_students_fee_pending_detail($id, $batch_id);
		
		//-get cheque values if 1st term pay 50% with no cheques 3rd term remaining 50% ( 50% + 50% 1 cheque) : for DIS
		$this->data['last_payment']  =  $this->Teststudents_model->get_students_last_payment_with_one_cheque($id, $batch_id);
		$this->data['first_term_fee']  =  $this->Teststudents_model->get_first_term_fee_amount($student[0]->course_id);
		...
		$time_elapsed_secs = microtime(true) - $start;
		$this->data['time_elapsed'] = $time_elapsed_secs;
		$this->load_template('settings/test_students/payment');
	}
	
	function payment_receive(){
		
		$this->Teststudents_model->insert_payment();
		redirect('test_students/student_profile/'.$_POST['student_id']."#tabs_3",'location');
	}
	
	function payment_receipt(){
		$payment_id = $this->uri->segment(3);
		$id = $_GET['student_id'];
		$this->data['student']  = $student = $this->Teststudents_model->get_students($id);
		$batch_id = $student[0]->batch_id;
		$this->data['pending_fee_list']  =  $this->Teststudents_model->get_students_fee_pending_detail($id, $batch_id, $student[0]->cdel);
		$this->data['payment']  = $payment = $this->Teststudents_model->get_student_payments($payment_id);
		$this->data['paid_trans']  = NULL;
		if( in_array($payment[0]->fee_desc, array('Tuition Fee', 'Education Plus Fee', 'Transportation Fee', 'Payment'))){
			$this->data['paid_trans']  =  $this->Teststudents_model->get_student_transactions($id, $batch_id, $payment[0]->fee_desc, $student[0]->cdel);
		}
		$this->load->view('settings/test_students/payment_receipt',$this->data);
	}
	...
	function payment_receipts(){
		...
		$students = $this->Teststudents_model->get_students_by_grade($course_id, $div_id, $p);
		foreach ($students as $student){ $i++;
			$this->data['student']  = $student;
			$id = $student->student_id;
			$batch_id = $student->batch_id;
			$this->data['pending_fee_list'] =  $this->Teststudents_model->get_students_fee_pending_detail($id, $batch_id, $student->cdel);
			$this->data['payments']  = $payments = $this->Teststudents_model->get_payments_by_student($id);
			foreach ($payments as $payment){
				...
				if( in_array($payment->fee_desc, array('Tuition Fee', 'Education Plus Fee', 'Transportation Fee', 'Payment'))){
					$this->data['paid_trans'][$payment->payment_id]  =  $this->Teststudents_model->get_student_transactions($id, $batch_id, $payment->fee_desc, $student->cdel);
				}
			}
			$this->load->view('settings/test_students/payment_receipts',$this->data);
		}
	}
	...
	function add_guarantee_cheque(){
		...
		$this->data['not_popup']  = FALSE;
		$this->load->view('settings/test_students/add_guarantee_cheque',$this->data);
	}
	
	function add_guarantee_cheque_update(){
		$this->Teststudents_model->insert_guarantee_cheque();
		$id = $_POST['student_id'];
		$batch_id = $_POST['batch_id'];
		$this->data['fee_paid_list']  =  $this->Teststudents_model->get_students_fee_paid_detail($id, $batch_id);
		$this->data['voucher_list']  =  $this->Teststudents_model->get_students_vouchers($id, $batch_id); // For refund details echo "Hi...";
	...
	function cheque_receipt(){
		...
		$this->load->view('settings/test_students/cheque_receipt',$this->data);
	}
	...
	function add_payment(){
		...
		$this->data['batch_list'] = Base_model::get_all_batches(1);
		$this->data['form'] = $student = $this->Teststudents_model->get_students($id);
		$this->data['subjects'] = $this->Teststudents_model->get_monthly_subjects_detail($student[0]->student_id, $student[0]->batch_id, $student[0]->month);
		$this->data['pending_amount']  =  $this->Teststudents_model->get_students_monthly_pending_fee($student[0]->student_id, $student[0]->batch_id, $student[0]->month);
		
		$this->data['not_popup']  = FALSE;
		$this->load->view('settings/test_students/add_payment',$this->data);
	}
	
	function add_payment_update(){
		$this->Teststudents_model->insert_monthly_payment();
		$id = $_POST['student_id'];
		$batch_id = $_POST['batch_id'];
		//$this->data['form'] = $student  =  $this->Teststudents_model->get_students($id);
		$this->data['outstanding']  =  $this->Teststudents_model->get_students_outstandings($id, $batch_id);
	...
	function get_subjects_fee(){
		...
		$subjects = $this->Teststudents_model->get_monthly_subjects_detail($_POST['student_id'], $_POST['batch_id'], $month);
		$pending_amount  =  $this->Teststudents_model->get_students_monthly_pending_fee($_POST['student_id'], $_POST['batch_id'], $month, 0);
		$sub_str = ''; $subjects_id = '';
		if(isset($subjects)){
		...
		$this->load->view('settings/test_students/add_payment_onchange_month',$this->data);
	...
	function add_discount(){
		...
		$this->data['not_popup']  = FALSE;
		$this->load->view('settings/test_students/add_discount',$this->data);
	}
	
	function add_discount_update(){
		$this->Teststudents_model->insert_discount();
		$id = $_POST['student_id'];
		$batch_id = $_POST['batch_id'];
		//$this->data['form'] = $student  =  $this->Teststudents_model->get_students($id);
		$this->data['outstanding']  =  $this->Teststudents_model->get_students_outstandings($id, $batch_id);
	...
	function holding_trans(){
		...
		$this->data['form']  =  $this->Teststudents_model->get_students($id);
		$this->data['values']  = $this->Teststudents_model->get_students_fee_paid_detail($id, $batch_id, $payment_id);
		$this->data['payment_id'] = $payment_id;
		
		$this->data['not_popup']  = FALSE;
		$this->load->view('settings/test_students/holding_transaction',$this->data);
	}
	
	function holding_trans_update(){
		$this->Teststudents_model->update_transaction();
		?>
	...
	function close_payment(){
		$payment_id = $this->uri->segment(3);
		$this->Teststudents_model->delete_transaction($payment_id);
	...
	function holdentries(){
		...
		$this->data['payment_list']  = $form = $this->Teststudents_model->get_all_hold_entries();
		$this->load->view('settings/test_students/holdentries',$this->data);
	}
	...
	function outstanding_detail(){
		...
		$this->data['batch_list'] = Base_model::get_all_batches(1);
		$this->data['not_popup']  = FALSE;
		$this->load->view('settings/test_students/outstanding_detail',$this->data);
	}
	...
	function edit_outstanding_data(){
		...
		$this->data['batch_list'] = Base_model::get_all_batches(1);
		$this->data['not_popup']  = FALSE;
		$this->load->view('settings/test_students/edit_outstanding_detail',$this->data);
	}
	
	function edit_outstanding_data_update(){
		
		$this->Teststudents_model->update_outstanding_data();
		$batch_id = $_POST['batch_id'];
		$outstanding = $this->Teststudents_model->get_students_outstandings($_POST['student_id'], $batch_id);
		...
		//$this->load->view('settings/test_students/updated_previous_detail',$this->data);
	}
	...
	function update_fields(){
		//print_r($_POST);
		$this->Teststudents_model->update_fileds();
		echo $_POST['value'];
	}
	...
	function assesment_slip(){
		...
		$this->data['student'] = $this->Teststudents_model->get_student_info($student_id);
		$this->data['assesment_list'] = Base_model::get_student_assesments($student_id);
		$this->load->view('settings/test_students/assesment_print',$this->data);
	}
	...
	function get_student_attendance(){
		...
		$detail_list = $this->Teststudents_model->get_attendance($st_id, $curr_year_month);
		$app_arr = array();
		if(isset($detail_list) && sizeof($detail_list) > 0){
			foreach($detail_list as $values){
				$ind = str_replace($curr_year_month.'-', '', $values->attendance_date);				
		...
		$this->load->view('settings/test_students/attendance_detail',$this->data);
	}
	...
	function add_attend(){
		...
		$this->load->view('settings/test_students/attend_mark',$this->data);
	}
	
	function add_attendance(){
		$obj = $this->Teststudents_model->set_attendance();
	...
	function add_parent_data(){
		...
		$this->data['not_popup']  = FALSE;
		$this->data['country_list'] = Base_model::get_country_list();
		$this->load->view('settings/test_students/add_parent_detail',$this->data);
	}
	
	function add_parent_data_insert(){
		$id = $_POST['student_id'];
		$this->Teststudents_model->insert_parent_data();
		$this->data['guardian']  =  Base_model::get_guardians($id);
		$this->load->view('settings/test_students/updated_parent_detail',$this->data);
	}
	...
	function edit_parent_data(){
		...
		$this->data['guardian']  = Base_model::get_guardians($id);
		$this->data['country_list'] = Base_model::get_country_list();
		$this->load->view('settings/test_students/edit_parent_detail',$this->data);
	}
	
	function edit_parent_data_update(){
		
		$id = $_POST['student_id'];
		$this->Teststudents_model->update_parent_data();
		$this->data['guardian']  =  Base_model::get_guardians($id);
		$this->load->view('settings/test_students/updated_parent_detail',$this->data);
	}
	...
	function add_previous_data(){
		...
		$this->data['not_popup']  = FALSE;
		$this->load->view('settings/test_students/add_previous_detail',$this->data);
	}
	
	function add_previous_data_insert(){
		$id = $_REQUEST['student_id'];
		$this->Teststudents_model->insert_previous_data();
		$this->data['previous_data']  =  Base_model::get_previous_data($id);
		$this->load->view('settings/test_students/updated_previous_detail',$this->data);
	}
	...
	function edit_previous_data(){
		...
		$this->data['not_popup']  = FALSE;
		$this->load->view('settings/test_students/edit_previous_detail',$this->data);
	}
	
	function edit_previous_data_update(){
		$id = $_REQUEST['student_id'];
		$this->Teststudents_model->update_previous_data();
		$this->data['previous_data']  =  Base_model::get_previous_data($id);
	...
	function update(){
		$id = $this->Teststudents_model->update();
		$d = $this->session->userdata(SESSION_CONST_PRE.'division_id');
		$division_name = ($d == 1) ? 'FB' : 'FG';
		...
		redirect('test_students/student_profile/'.$_POST['student_id'],'location');
	}
	...
	function delete(){
		...
		if(isset($_GET['selected_id']) && !empty($_GET['selected_id'])){
			...
			if($admin_role  >= 3){
				$this->Teststudents_model->delete($_GET['selected_id']);
			}
			redirect('test_students','location');
		}
	}
	...
	function enable_student(){
		...
		if($admin_role >= 3){
			$id = $this->uri->segment(3);
			$this->Teststudents_model->activate_student($id);
			redirect('test_students/student_profile/'.$id,'location');
		}
		redirect('test_students','location');
	}
	...
	function imgcrop(){
		...
		$dir_path = './assets/uploads/test_students/';
	...
	function view_file(){
		...
		$this->data['file']  = $this->Teststudents_model->get_attached_file($_GET['f']);
		
		$this->load->view('settings/test_students/view_file', $this->data);
	}
	...
	function download_file(){
		...
		$this->load->view('settings/test_students/download_file', $this->data);
	}
	
	function download_zip(){
		$id = $this->uri->segment(3); // lecture_id
		$this->data['form']  =  $this->Teststudents_model->get_student_files($id);
		$this->data['student_id']  = $id;
		$this->load->view('settings/test_students/download_zip', $this->data);
	}
	
	function cron_process(){
		$check = $this->Teststudents_model->add_trem_fee();
		if($check){
	...
	function print_view(){
		...
		$id = $this->uri->segment(3);
		$this->data['form'] = $form =  $this->Teststudents_model->get_students($id);
		...
		$div_id = $this->session->userdata(SESSION_CONST_PRE.'division_id');		
		$this->load->view('settings/test_students/print_view'.$div_id, $this->data);
	}
}