...
class test_marksheet extends Base_Controller{
	...
	function __construct()
	{
		...
		$this->load->model('test_marksheet_model','',TRUE);
	...
	function index(){
		...
		if($admin_role == 1){
			$this->data['student_list'] = $this->test_marksheet_model->get_student_by_class_section($course_id, $section);
			$this->data['subject_list'] = $this->test_marksheet_model->get_all_subjects();
			$this->data['courses_list'] = Base_model::get_all_courses();
		}
		else{
				
			$this->data['subject_list'] = $ss = $this->test_marksheet_model->get_subject_assigned();
			$this->data['courses_list'] = $cc = $this->test_marksheet_model->get_courses_by_subject($subject_id);
		...
		$this->data['student_list'] = $mark_detail = $this->test_marksheet_model->get_marks($subject_id, $course_id, $section, $batch, $term);
	...
	function add_marks(){
		...
		{
			$row = $this->test_marksheet_model->get_header_info($_GET['course_id'], $_GET['sid']);
		...
		$student = $this->test_marksheet_model->get_student_info($_GET['student_id']);
		...
		$this->data['marks'] = $mark = $this->test_marksheet_model->get_subject_marks($_GET['student_id'], $_GET['sid'], $course, $section, $batch, $_GET['t']);
		//print_r($mark);
	...
	function add_marksave(){
		if(isset($_POST['f']) && $_POST['f'] == 'up'){
			$this->test_marksheet_model->delete_marks();
			$obj = $this->test_marksheet_model->insert_marks();
			$this->marksheet_list();
	...
	function update_marks(){
		...
		$section = isset($_GET['section']) ? $_GET['section'] : $this->session->userdata(SESSION_CONST_PRE.'section');
		//$student_id, $subject_id, $course_id, $section, $batch, $term
		$this->data['marks'] = $mark = $this->test_marksheet_model->get_subject_total_marks($course, $batch, $_GET['t']);
		//print_r($mark);
	...
	function add(){
		...
		if($this->form_validation->run() == TRUE){
			$this->test_marksheet_model->insert();
	...
	function edit(){
		...
		$this->data['company_list'] = Base_model::get_all_companies();
		$this->data['form']  =  $this->test_marksheet_model->get_timetable($id);
	...
	function print_view(){
		...
		$this->data['student_list'] = $mark_detail = $this->test_marksheet_model->get_marks($subject_id, $course_id, $section, $batch, $term);
		$this->data['subject'] = $this->test_marksheet_model->get_subject_name($subject_id);
		$this->data['courses_list'] = Base_model::get_all_courses();
	...
	function attendance_list(){
		...
		$this->data['student_list']   = $student_list   = $this->test_marksheet_model->get_marks($subject_id, $course, $section, $batch, $term);
		$this->data['student_list10'] = $student_list10 = $this->test_marksheet_model->get_marks($subject_id, $course+1, $section, $batch, $term);
		if(sizeof($student_list) > 30 || sizeof($student_list10) > 30){
			$this->data['student_list']   =  $this->test_marksheet_model->get_marks($subject_id, $course_id, $section, $batch, $term);	
	...
	function send_sms(){
		...
		if(isset($_GET['course_id']) && !empty($_GET['course_id'])){
			...
			$student_list = $this->test_marksheet_model->get_marks($subject_id, $course_id, $section, $batch, $term);
			$subject = $this->test_marksheet_model->get_subject_name($subject_id);
			...
			{	
				...
				if( isset($student->cell_phone_father) && $student->obtained_marks > 0 ){
		...
		//redirect('test_marksheet?msg=1','location');
	}
}