...
class attendance_report extends Base_Controller{
	...
	function index(){
		...
		$this->data['form_detail']=$this->Reports_model->get_attendance_report($from, $to);
		$this->data['model'] = 'attendance_report';
	...
	function search(){		
		...
		$this->data['form_detail']=$this->Reports_model->get_attendance_report($from, $to, $_POST);
		$this->load->view('reports/attendance_report',$this->data);
	}
	...
	function prints(){
		...
		$this->data['form_detail']=$this->Reports_model->get_attendance_report($from, $to, $_GET);
		$this->data['model'] = 'attendance_report';
		$this->load_printhtml_tmpl('reports/attendance_report');
	}
	...
	function excel(){
		...
		$search_date = Base_Controller::ToggleLang('Attendance Date').': '. Util::displayFormat($from) .' to '. Util::displayFormat($to) .' ';
		
		$form_detail = $this->Reports_model->get_attendance_report($from, $to, $_GET);
		$this->data['model'] = 'attendance_report';
		...
		$this->excel->getActiveSheet()->setCellValue('A1', 'Attendance Report (Absent Student)');
		...
		$this->excel->getActiveSheet()->getStyle('F2')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
		$this->excel->getActiveSheet()->getStyle('A2')->getFont()->setBold(true);
		$this->excel->getActiveSheet()->getStyle('F2')->getFont()->setBold(true);
		$this->excel->getActiveSheet()
		->setCellValue('A2', $search_date)
		->setCellValue('F2', 'DATE:'.date('d-m-Y H:i:s'));
		...
		$this->excel->getActiveSheet()->getStyle('F3')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
		$this->excel->getActiveSheet()->getStyle('A3')->getFont()->setBold(true);
		$this->excel->getActiveSheet()->getStyle('F3')->getFont()->setBold(true);
		$this->excel->getActiveSheet()
		->setCellValue('A3', $search_string)
		->setCellValue('E3', 'Total Students: '.sizeof($form_detail));
		...
		->setCellValue('F4', 'Father Contact')
		->setCellValue('G4', 'Mother Contact');
		/*---------------------------------------------------------------------------------*/
		...
		if(isset($form_detail) && sizeof($form_detail) > 0){
			foreach($form_detail as $values){
				...
				->setCellValue('E'.$i, $values->attendance_date)
				->setCellValue('F'.$i, $values->cell_phone_father)
				->setCellValue('G'.$i, $values->cell_phone_mother);
	
			}
		}
		/*---------------------------------------------------------------------------------*/
		$filename='attendance_report_'.date('Ymd').'.xls'; //save our workbook as this file name
