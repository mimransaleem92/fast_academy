<?php
class Resultsheet_model extends Base_Model{
	...
	function get_marks($course_id, $section, $batch, $term){
		...
		$this->db->select('s.student_name, m.*');
		$this->db->from('students s');
		$this->db->join('student_subject_monthly m','s.student_id = m.student_id');
		$this->db->where(array("s.course_id" => $course_id, 's.section'=>$section, 'm.batch_id'=>$batch, 'm.month'=>$term));
	...
	function get_student_result($student_id, $batch, $term){
		...
		$this->db->join('subject_monthly_marks smm','smm.subject_id = sb.subject_id and smm.month = m.`month` and smm.batch_id = m.batch_id and smm.course_id = s.course_id');
		$this->db->where(array('m.student_id'=>$student_id, 'm.batch_id'=>$batch, 'm.month'=>$term));
		$this->db->where('s.division_id', $div_id);
		$this->db->where('s.cdel', '0');
	...
	function insert_marks(){
		...
		$this->db->insert_batch('subject_monthly_marks', $values);
	}
	...
	function delete_marks(){
		...
		$this->db->delete('subject_monthly_marks');
	}
	...
	function get_subject_total_marks($course_id, $batch_id, $term, $subject_id = ''){
		...
		$this->db->from('subject_monthly_marks sm');
	...
	function insert_subject_total_marks($course_id, $batch_id, $term){
		$sql =  "INSERT INTO subject_monthly_marks (course_id, batch_id, month, subject_id, total_marks)
