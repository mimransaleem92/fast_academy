...
class Teststudents_model extends Base_Model{
	...
	function insert()
    {
    	...
    	if(ENABLE_FEE_MODULE == 1){	
			...
			$record = array('student_id'=>$student_id, 'batch_id'=>$batch_id);
			...
			{    	
				...
				if($course_id > 10 && ($subject_id == 19 || $subject_id == 1)){
					$due_fee=$subject_fee * (2/3);
				}else
				{
					$due_fee=$subject_fee;
				}
				...
				for($m = 1; $m <= 12; $m++){
					$sub = array('subject_id'=> $subject_id, 'month'=>$m);
			...
			if($this->db->insert_batch('test_student_subject_monthly', $receipt)){
	...
	function get_due_payments($student_id){
		$this->db->select('sp.subject_id, sp.due_amount');
		$this->db->from('students s');
		$this->db->join('student_payments sp', 's.student_id = sp.student_id AND s.month=sp.month');
		$this->db->where(array("s.student_id"=>$student_id, "due_amount >" => 0, 'sp.month <>'=>0, 'sp.mark_delete'=>0));
		$this->db->order_by('sp.subject_id');
		$query = $this->db->get();
		//echo $str = $this->db->last_query();
		return $query->result();
	}
	...
	function insert_monthly_payment(){
		...
		$due_payments = $this->get_due_payments($_POST['student_id']);
		$total = 0;
		foreach($due_payments as $payment){
			$total += $payment->due_amount;
		}
		
		foreach($due_payments as $sp){
			$subject_fee = ($sp->due_amount / $total) * $_POST['payment_amount'];
			$student_payments[] = array_merge( $payment_arr , array('subject_id'=> $sp->subject_id, 'payment_amount'=>$subject_fee, 'payment_date'=>$payment_date));
		}
		//print_r($student_payments);
		$this->db->insert_batch('student_payments', $student_payments);
	}
	...
	function insert_student_subjects_monthly(){
		...
		foreach ($result as $row) {
			...
			//$this->db->insert('test_student_subject_monthly', $order);
		...
		//print_r($receipt);
		$this->db->insert_batch('test_student_subject_monthly', $receipt);
	...
	function insert_student_payments_monthly(){
		...
		foreach ($result as $row) {
			...
			$this->db->from('test_student_subject_monthly');
	...
	function update_monthly_subjects($id, $batch_id, $course_id, $month){
		...
		$subject_fee = $this->get_fee_per_subject($course_id);
		if(isset($_POST['test_started'])){
			$this->db->where(array('student_id'=>$id, 'batch_id'=>$batch_id, 'month >='=>$_POST['test_started']));
			$this->db->delete('test_student_subject_monthly');
			...
			{    	
				...
				for($m = $_POST['test_started']; $m <= 12; $m++){
					$sub = array('subject_id'=> $subject_id, 'month'=>$m);
					$receipt[] = array_merge( $record , $sub);
				}
				$student_payments[] = array_merge( $record1 , array('subject_id'=> $subject_id, 'month'=>$month, 'due_amount'=>$due_fee));
	...
	function get_monthly_subjects_detail($student_id, $batch_id, $month){
		...
		$this->db->from('test_student_subject_monthly ss');
	...
	function add_student_tests(){
	    $receipt = array();
	    $batch_id = $this->session->userdata(SESSION_CONST_PRE.'batch_id');
	    $this->db->from('test_student_subject_monthly')->where('month', 1);
	    $this->db->order_by('student_id');
	    $query = $this->db->get();
	    $result = $query->result();
	    ...
	    foreach($result as $row){
	        ...
	        for($m = 2; $m <= 11; $m++){
    	        ...
    	        $receipt[] = array('student_id'=>$student_id, 'batch_id'=>$batch_id, 'subject_id'=> $row->subject_id, 'month'=>$m);
    	    }
	    }
	    //print_r($receipt);
	    $this->db->insert_batch('test_student_subject_monthly', $receipt);
