<?php
class Gazettesheet_model extends Base_Model{
	...
	function get_marks($course_id, $section, $batch, $term){
		$div_id = $this->session->userdata(SESSION_CONST_PRE.'division_id');
		$this->db->select('s.student_name, m.*, sum(m.obtained_marks) as marks, sum(m1.total_marks) as subject_total');
		$this->db->from('students s');
		$this->db->join('test_student_subject_monthly m', "s.student_id = m.student_id and obtained_marks != ''");
		$this->db->join('test_subject_monthly_marks m1', "m.subject_id = m1.subject_id AND m.batch_id = m1.batch_id AND m.`month` = m1.`month` and m1.course_id = $course_id");
		$this->db->where(array("s.course_id" => $course_id, 's.section'=>$section, 'm.batch_id'=>$batch));
	...
	function get_student_result($student_id, $batch, $term){
		$div_id = $this->session->userdata(SESSION_CONST_PRE.'division_id');
	...
	function get_subject_total_marks($course_id, $batch_id, $term, $subject_id = ''){
		...
		$this->db->select("sm.*, sum(sm.total_marks) as total_marks, s.subject_name, s.subject_code", FALSE);
		$this->db->from('test_subject_monthly_marks sm');
		$this->db->join('subjects s',"s.subject_id = sm.subject_id AND is_active='Y'");
		$this->db->where(array("batch_id" => $batch_id, 'course_id'=>$course_id));
		$this->db->group_by('sm.subject_id');
		if($subject_id != ''){
